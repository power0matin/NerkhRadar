...
async def check_alerts(bot: Bot):
    ...
    for user in users:
        ...
        for alert in alerts:
            current_price = await get_dollar_price() if alert["asset"] == "dollar" else await get_crypto_price(alert["asset"])
            if (alert["condition"] == ">" and current_price > alert["value"]) or \
               (alert["condition"] == "<" and current_price < alert["value"]):
                await bot.send_message(user["id"], f"⚠️ هشدار: {alert['asset']} به {current_price} رسید!")
