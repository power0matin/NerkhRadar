...
async def get_dollar_price():
    async with aiohttp.ClientSession() as session:
        async with session.get("https://www.tgju.org/") as response:
            ...
            price = soup.find("div", class_="price").text.strip()  # Adjust selector as needed
...
async def get_gold_prices():
    async with aiohttp.ClientSession() as session:
        async with session.get("https://www.tgju.org/") as response:
            soup = BeautifulSoup(await response.text(), "html.parser")
            return {
                "gram": float(soup.find("div", class_="gold-gram").text.replace(",", "")),
                "coin": float(soup.find("div", class_="gold-coin").text.replace(",", "")),
                "ounce": float(soup.find("div", class_="gold-ounce").text.replace(",", ""))
            }
